...
- **Acceso: ya es real (Supabase Auth), desde esta sesión.** El contrato sigue en `src/auth/tipos.ts` (`AccionesDeAcceso`/`AccionesDeCuenta`); `src/auth/accionesReales.ts` lo implementa contra `supabase.auth.*`, y `SesionContext` arma `usuario`/`rol`/`enRecuperacion` escuchando un único `onAuthStateChange` (ningún action actualiza ese estado a mano: cada uno ya cambia la sesión de Supabase, y eso es lo que dispara el evento). `flowType: 'pkce'` en `src/lib/supabase.ts`: los enlaces vuelven con `?code=`, no con el token en el hash. Las respuestas de registro y recuperación siguen siendo iguales exista o no el email (es el comportamiento por defecto de Supabase con "Confirm email" activo). Solo se vuelve a una ruta interna (`destinoSeguro`), y el email viaja por `location.state`, nunca por la URL.
  ...
  - **Puente temporal con el catálogo, todavía simulado**: `contenido.ts`/`admin.ts` (el catálogo público y el panel) siguen leyendo datos de `mock.ts` y deciden el acceso mirando `rolSimulado()` (`src/datos/sesionSimulada.ts`). `SesionContext` mantiene esa variable sincronizada con el rol real después de cada cambio de sesión, así el panel no se ve vacío para una admin de verdad mientras no se haga la Tanda de datos. Se borra entero (el puente y `sesionSimulada.ts`) cuando esa tanda conecte esas dos capas a Supabase. Una consecuencia visible: en un build de producción real (sin `VITE_DATOS_DE_PRUEBA`) el catálogo sale vacío hasta esa tanda, porque el mock no se emite.
  ...
  - **URL de retorno**: hoy apunta a `http://localhost:5173` (Site URL y Redirect URLs en *Authentication → URL Configuration*). Cuando el sitio tenga dominio en Vercel, hay que agregar esa URL nueva ahí (un campo más en la lista de redirects; no reemplaza la de localhost, así el desarrollo local sigue andando).
