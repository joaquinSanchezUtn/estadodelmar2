...
- **Suscripción y acceso**: una tabla `suscripciones` (`preapproval_id` único, `estado`, `acceso_hasta`, `user_id`) en lugar del booleano `suscripcion_activa`, que no puede representar «cancelada con acceso hasta X». `tiene_acceso()` compara la fecha en cada consulta (`cancelada` y `acceso_hasta >= current_date`): Mercado Pago no avisa cuando termina el período, así que ningún webhook lo va a cortar. Las transiciones se validan en el servidor: cancelar exige `activa`; reactivar exige `cancelada` con período vigente.
  - `obtenerSuscripcion()` (ya real, en `contenido.ts`) mapea `iniciada`→`pendiente` y `en_gracia`→`cancelada` porque el front todavía no tiene pantalla propia para esos dos casos, y hoy no hay ninguna fila real con esos estados (nada escribe en `suscripciones` todavía). Antes de que el webhook exista, `SuscripcionCancelada.tsx` va a necesitar distinguir "cancelaste vos" de "te falló el cobro, tenés 3 días" — son mensajes y botones distintos.
...
- **Reproductor**: la URL firmada devuelve su duración (`duracionSeg`) y no un instante absoluto. `init_point` de Mercado Pago validado en el servidor (solo mercadopago.com). `obtenerEstadoDelPago` filtra por `user_id`.
